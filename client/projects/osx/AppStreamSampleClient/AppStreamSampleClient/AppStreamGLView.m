...
@interface AppStreamGLView()
- (void) initGL;
@end


@implementation AppStreamGLView
...
{
    CVReturn result = [(__bridge AppStreamGLView*)displayLinkContext getFrameForTime:outputTime];
    return result;
}
...
-(void) sharedInit
{
    NSOpenGLPixelFormatAttribute attrs[] =
    {
        ...
        NSOpenGLPFADepthSize, 0, //No depth buffer needed
...
- (void) prepareOpenGL
{
    ...
    // additionally shut down the displayLink if we get this notification from xstx
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(stopDisplayLink:)
                                                 name:kXSTX_CLIENT_STOPPED_NOTIFICATION
                                               object:nil];
    
    //But make sure we start the display link when we get the client ready notification
...
- (void) stopDisplayLink:(NSNotification*)notification
{
...
//    NSLog(@"Drawing view");
    ...
    [[AppStreamClient sharedClient] renderFrame];
