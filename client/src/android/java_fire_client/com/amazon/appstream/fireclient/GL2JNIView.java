...
class GL2JNIView extends GLSurfaceView {
    ...
    private static int mSmallHeight = Integer.MAX_VALUE;

    private static class Renderer implements GLSurfaceView.Renderer {
        public void onDrawFrame(GL10 gl) {
            AppStreamInterface.step();
        }

        public void onSurfaceChanged(GL10 gl, int width, int height) {
            mHeight = Math.max(mHeight,height);
            mSmallHeight = Math.min(mSmallHeight,height);

            AppStreamInterface.setupGraphics(width, height);
        }
    ...
    public int getHeightDelta() { return mHeight - mSmallHeight; }
}
