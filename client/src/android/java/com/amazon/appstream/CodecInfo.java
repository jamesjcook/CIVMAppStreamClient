 ...
 *
 * Original source:
 *   https://android.googlesource.com/platform/cts/+/master/suite/cts/deviceTests/videoperf/src/com/android/cts/videoperf/CodecInfo.java
 */
...
public class CodecInfo {
    ...
    /** if AVC, then the highest AVC supported level */
    public int mLevel = 0;

    /** if AVC, then the AVC supported profile */
        ...
        if (mimeType.equals(VIDEO_AVC)) {
            ...
            for (CodecProfileLevel lvl : cap.profileLevels) {
                if (lvl.level > highestLevel) {
                    ...
                    info.mProfile = lvl.profile ;
                }  else if (lvl.level == highestLevel) {

                    if (lvl.profile == CodecProfileLevel.AVCProfileHigh444) {
                        // prefer high444 profile
                        info.mProfile = lvl.profile;
                    } else if (lvl.profile == CodecProfileLevel.AVCProfileHigh &&
                               info.mProfile != CodecProfileLevel.AVCProfileHigh444) {
                        // second choice, high profile
                        info.mProfile = lvl.profile;
                    } else if (info.mProfile==CodecProfileLevel.AVCProfileBaseline) {
                        // if we're currently baseline, then pick something else
                        info.mProfile = lvl.profile;
                    }
                }
            }
            info.mLevel = highestLevel;
