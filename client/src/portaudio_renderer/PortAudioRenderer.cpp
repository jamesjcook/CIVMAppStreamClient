...
PortAudioRenderer::~PortAudioRenderer()
{
    // If we've got a portAudioStream, Pa_Initialize has succeeded,
    // and Pa_OpenStream has succeeded. We need to close the stream,
    // and call Pa_Terminate to clean up. If portAudioStream is NULL,
    // we don't need to call Pa_Terminate. It's either already been
    // called or Pa_Initialize hasn't been called.
    if (NULL != mPortAudioStream) 
    {
        PaError err = Pa_CloseStream(mPortAudioStream);
 ...
 */
XStxRawAudioFrame* PortAudioRenderer::popFrame(int delay, int msBuffer)
{
 ...
 */
XStxResult PortAudioRenderer::start()
{
    if (!mDidInit)
    {
    ...
    if (!mAudioIsPlaying)  
    {
        PaError err = Pa_StartStream(mPortAudioStream);
        if(paNoError != err) 
        {
 ...
 */
void PortAudioRenderer::pause(bool pause)
{
...
bool PortAudioRenderer::initializePortAudio()
{
    ...
    // open PortAudio stream with configured parameters
    err = Pa_OpenStream(
                &mPortAudioStream,
                NULL,
                &paStreamParams,
                SAMPLING_RATE,
                    // we want PortAudio to give us Buffers of the right size, 
    ...
    // successfully initialized PortAudio stream
    return true;
 ...
 */
bool PortAudioRenderer::fillPABuffer(
                            int16_t* buffer, 
...
/** PortAudio callback */
int PortAudioRenderer::paStreamCallback(
                            ...
                            void *userData )
{
    PortAudioRenderer* renderer = (PortAudioRenderer*) userData;
